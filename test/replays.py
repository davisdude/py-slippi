...
class TestGame(unittest.TestCase):
        ...
        self.assertEqual(
            ...
            Metadata(
                date=datetime.datetime(
                    2018, 6, 22, 7, 52, 59, 0, datetime.timezone.utc
                ),
                duration=5209,
    ...
    def test_ics(self):
        game = self._game("ics")
        self.assertEqual(
            game.metadata.players[0].characters,
            {InGameCharacter.NANA: 344, InGameCharacter.POPO: 344},
        )
    ...
    def test_ucf(self):
        self.assertEqual(
            self._game("shield_drop").start.players[0].ucf,
            Start.Player.UCF(dash_back=False, shield_drop=True),
        )
        self.assertEqual(
            self._game("dash_back").start.players[0].ucf,
            Start.Player.UCF(dash_back=True, shield_drop=False),
        )
    ...
    def test_netplay_name(self):
        ...
        self.assertEqual(
            players[0].netplay,
            Metadata.Player.Netplay(code="ABCD#123", name="abcdefghijk"),
        )
        self.assertEqual(
            players[1].netplay, Metadata.Player.Netplay(code="XX#000", name="nobody")
        )
    ...
    def test_unknown_event(self):
        ...
        self.assertEqual(
            log_context.output, ["INFO:slippi.log:ignoring unknown event type: 0xff"]
        )
