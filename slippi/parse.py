...
def _parse_event(event_stream, payload_sizes):
    ...
    try:
        base_pos = event_stream.tell() if event_stream.seekable() else None
    ...
    try:
        ...
        if event_type is EventType.GAME_START:
            ...
            event = Frame.Event(
                Frame.Event.PortId(stream), Frame.Event.Type.PRE, stream
            )
        elif event_type is EventType.FRAME_POST:
            event = Frame.Event(
                Frame.Event.PortId(stream), Frame.Event.Type.POST, stream
            )
        elif event_type is EventType.FRAME_START:
            event = Frame.Event(Frame.Event.Id(stream), Frame.Event.Type.START, stream)
        elif event_type is EventType.ITEM:
            event = Frame.Event(Frame.Event.Id(stream), Frame.Event.Type.ITEM, stream)
        elif event_type is EventType.FRAME_END:
            event = Frame.Event(Frame.Event.Id(stream), Frame.Event.Type.END, stream)
        ...
        if isinstance(event, Start):
            ...
            if skip_frames:
                skip = (
                    total_size
                    - bytes_read
                    - payload_sizes[EventType.GAME_END.value]
                    - 1
                )
            ...
            if (
                event.type is Frame.Event.Type.PRE
                or event.type is Frame.Event.Type.POST
            ):
...
def _parse_try(input: BinaryIO, handlers, skip_frames):
    ...
    try:
        ...
        try:
            e.filename = input.name  # type: ignore
        except AttributeError:
            pass
...
def parse(
    input: Union[BinaryIO, str, os.PathLike],
    handlers: Dict[ParseEvent, Callable[..., None]],
